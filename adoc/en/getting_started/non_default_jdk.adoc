...
--
Both IntelliJ IDEA and Gradle by default use Java Development Kit (JDK) defined by the `JAVA_HOME` environment variable to assemble and run Java projects. In order to use the non-default JDK in your project you need to change quite a few settings.
...
* Add JDK 11 to the global list of IDEA SDKs if it's hasn't been added yet. Go to the Main menu -> *File* -> *Project Structure*. Select *SDKs* in the left menu. Press *+* button -> choose *JDK* -> select JDK 11 installation folder. Press *OK* to save changes.
