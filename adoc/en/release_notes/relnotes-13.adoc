...
.. The *Restore failed project to the latest stable state* item reverts Gradle build script to the state after last successful import, which helps with unintended or erroneous build script changes.
+
image::release_notes/v-13-menu-import-failed.png[align="center"]

. Standalone IDE version of the Studio has been upgraded to IntelliJ Community platform *2019.2*.
